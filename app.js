...
amqp.connect("amqp://esys:esys@cloud.faps.uni-erlangen.de", function(err, conn) {
    ...
    amqp_connection.createChannel(function(err, ch) {
        ...
        amqp_ch.assertExchange("FAPS_DEMONSTRATOR_CurrentEnergyPrice", 'fanout', { durable: false });
        // test client
        amqp_ch.assertQueue('FAPS_DEMONSTRATOR_LiveStreamData_MachineData_VisQueue', { exclusive: true }, function(err, q) {
            if (err) {
                ...
                ch.consume(q.queue, function(msg) {
                    ...
                    io.emit('AMQPMachineData', {
                        'robot': JSON.parse(msg.content.toString()),
...
/* Energy Price API
/*#####################################################################################*/
var simulateEnergy = false;
var dateFormat = require('dateformat');
const superagent = require('superagent');
var current_energy_price = 1.0;
const ENERGY_PRICE_TIME_INTERVAL = 10000; // 10 s
var request_interval = setInterval(function(){
    const _url = "https://hourlypricing.comed.com/rrtp/ServletFeed?type=daynexttoday&date=" +  dateFormat(new Date(), "yyyymmdd");
    superagent.get(_url, (err, res) => {
        if (err){
            logger.error('ENERGY Price API Error: ' + err);
        }else{            
            if (simulateEnergy === false){
                var series1 = eval(res.text);
                current_energy_price = series1[series1.length -1][1];
            }
            
        }
        io.emit('CurrentEnergyPrice', current_energy_price);
        // Publish to RabbitMq
        amqp_ch.publish("FAPS_DEMONSTRATOR_CurrentEnergyPrice", '', Buffer.from(JSON.stringify({'current_energy_price':current_energy_price})));
    });
}, ENERGY_PRICE_TIME_INTERVAL);
...
app.get('/EnergyPriceSimulation', function (req, res) {
    simulateEnergy = eval(req.query.simulate || simulateEnergy);
    current_energy_price = eval(req.query.price || current_energy_price);
